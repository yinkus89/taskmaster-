...
const validateInput = (data) => userValidationSchema.validate(data, { abortEarly: false });
...
router.post("/signup", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: "User creation failed due to a server error", error: error.message });
  }
});
...
router.post("/login", rateLimiter, async (req, res) => {
  ...
  try {
    ...
    // Generate a token
    const token = jwt.sign({ userId: user._id, email: user.email }, process.env.JWT_SECRET, { expiresIn: "30h" });
...
router.put("/tasks/:id", authenticate, ownershipMiddleware, async (req, res) => {
...
router.delete("/tasks/:id", authenticate, ownershipMiddleware, async (req, res) => {
  const taskId = req.params.id;
